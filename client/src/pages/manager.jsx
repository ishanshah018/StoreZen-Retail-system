...
class Trie {
    ...
    const processSalesData = (bills, products) => {
        ...
        bills.forEach(bill => {
            ...
            // Payment mode stats - map old payment methods to new ones
            let paymentMethod = bill.paymentMethod;
            
            // Migration: Convert old payment methods to new schema
            if (paymentMethod === 'Card' || paymentMethod === 'UPI' || paymentMethod === 'Wallet') {
                paymentMethod = 'Online'; // Convert old online payment methods to 'Online'
            }
            
            if (paymentModes.hasOwnProperty(paymentMethod)) {
                paymentModes[paymentMethod] += bill.billing.finalAmount;
            }
                            ...
                            return (
                                ...
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <div className={`${colors[index % colors.length]} rounded-full p-2 mr-4`}>
                                        ...
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={iconMap[method] || iconMap['Cash']}/>
                                        </svg>
                                    </div>
                                    <div>
